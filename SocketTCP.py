...
class SocketTCP:
    ...
    def connect(self, address):
        ...
        while time_to_wait > 0:
            print("aqui")
            try:
                msg, _ = self.socket.recvfrom(16)
                ack_header = self.parse_segment(msg.decode())
                if header["SYN"] and ack_header["ACK"] and ack_header["SEQ"]==self.seq-1:
                    self.socket.sendto(ack_segment.encode(), self.address_destiny)
                    print("Enviando mensaje: ", ack_segment)
                elif ack_header["SEQ"]==self.seq:
                    time_to_wait = 0
            except socket.timeout:
                time_to_wait -= 1
